<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\Ticket $ticket
 */
?>
<div class="row">
    <aside class="column">
        <div class="side-nav">
            <h4 class="heading"><?= __('Actions') ?></h4>
            <?= $this->Form->postLink(
                __('Delete'),
                ['action' => 'delete', $ticket->id],
                ['confirm' => __('Are you sure you want to delete # {0}?', $ticket->id), 'class' => 'side-nav-item']
            ) ?>
            <?= $this->Html->link(__('List Tickets'), ['action' => 'index'], ['class' => 'side-nav-item']) ?>
        </div>
    </aside>
    <div class="column column-80">
        <div class="tickets form content">
            <?= $this->Form->create($ticket) ?>
            <fieldset>
                <legend><?= __('Edit Ticket') ?></legend>
                <?php
                    echo $this->Form->control('name');
                    echo $this->Form->control('description');
                    echo $this->Form->control('price');
                ?>
                    <?= $this->Form->label('category', 'Ticket Category', ['class' => 'control-label']) ?>
                    <?= $this->Form->select('category', $ticketCategory, ['empty' => true, 'label' => 'Select Category', 'class' => 'form-control']) ?>
    
                    <?= $this->Form->label('event', 'Event', ['class' => 'control-label']) ?>
                    <?= $this->Form->select('event', $events, ['empty' => true, 'label' => 'Select Event', 'class' => 'form-control']) ?>
                <?php 
                    // echo $this->Form->control('category');
                    // echo $this->Form->control('event');
                    echo $this->Form->control('total_quantity');
                    echo $this->Form->control('avilable_quantity');
                    echo $this->Form->control('max_purchase_value');
                    // echo $this->Form->control('expiry');
                    // echo $this->Form->control('created_at');
                    // echo $this->Form->control('updated_at');
                ?>

            <?= $this->Form->control('expiry', ['type'=>'datetime','label' => 'Ticket expiry timestamp','placeholder' => 'Enter ticket expiry timestamp']) ?>

            </fieldset>
            <?= $this->Form->button(__('Submit')) ?>
            <?= $this->Form->end() ?>
        </div>
    </div>
</div>
